...
func (p *Parser) parseFnDecl() (*ast.FnDecl, error) {
	...
	if !p.check(token.TK_I32) && !p.check(token.TK_BOOL) && !p.check(token.TK_STRING_TYPE) {
		return nil, fmt.Errorf("line %d: syntax error, expected return type 'i32', 'bool' or 'string'", p.peek().Line)
	}
...
func (p *Parser) parseParams() ([]ast.Param, error) {
	...
	for {
		...
		if !p.check(token.TK_I32) && !p.check(token.TK_BOOL) && !p.check(token.TK_STRING_TYPE) {
			return nil, fmt.Errorf("line %d: syntax error, expected parameter type 'i32', 'bool' or 'string'", p.peek().Line)
		}
		typeTok := p.advance()
...
func (p *Parser) parseLet() (ast.Stmt, error) {
	...
	if !p.check(token.TK_I32) && !p.check(token.TK_BOOL) && !p.check(token.TK_STRING_TYPE) {
		return nil, fmt.Errorf("line %d: syntax error, expected type 'i32', 'bool' or 'string'", p.peek().Line)
	}
	p.advance()
